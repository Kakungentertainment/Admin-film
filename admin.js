...
let inboxTabActive=false,inboxRenderToken=0,inboxRenderTimer=null;
const ALL_GENRES=[
...
db.ref('inbox').on('value',snap=>{
  INBOX=snap.val()||{};
  updateInboxBadge();
  // Jangan render seluruh Inbox setiap Firebase berubah jika tab Inbox sedang tidak dibuka.
  // Saat terbuka, render ulang secara bertahap agar UI tetap responsif.
  if(inboxTabActive) scheduleInboxRender();
});
...
function switchAdmTab(t){
  inboxTabActive = t==='inbox';
  inboxRenderToken++;
  if(inboxRenderTimer){clearTimeout(inboxRenderTimer);inboxRenderTimer=null;}
  ...
  if(t==='inbox')scheduleInboxRender();
...
function scheduleInboxRender(){
  if(!inboxTabActive)return;
  const token=++inboxRenderToken;
  if(inboxRenderTimer)clearTimeout(inboxRenderTimer);
  inboxRenderTimer=setTimeout(()=>{
    inboxRenderTimer=null;
    renderInboxList(token);
  },40);
}

function renderInboxList(token=inboxRenderToken){
  const l=document.getElementById('inboxList');
  if(!l||!inboxTabActive||token!==inboxRenderToken)return;
  l.innerHTML='';
  ...
  const renderChunk=()=>{
    if(!inboxTabActive||token!==inboxRenderToken)return;
    const frag=document.createDocumentFragment();
    const end=Math.min(index+CHUNK,keys.length);

    for(;index<end;index++){
      const key=keys[index],item=INBOX[key],thread=item.thread||[];
    ...
    l.appendChild(frag);
    // Scroll hanya pada thread yang baru dibuat, bukan seluruh Inbox.
    Array.from(l.children).slice(Math.max(0,index-CHUNK),index).forEach(d=>{
      const ct=d.querySelector('.chat-thread');
